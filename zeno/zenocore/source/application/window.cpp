...
namespace zeno
{
    window_t::window_t(const char* window_name, int32_t pos_x, int32_t pos_y, int32_t size_x, int32_t size_y, HINSTANCE instance, int32_t show_command)
    :
    _title(window_name),
    _pos_x(pos_x),
    _pos_y(pos_y),
    ...
    void window_t::create_window(int32_t show_command)
    {
        _handle = CreateWindowExA(
            ...
            MAKEINTATOM(_class_atom), _title,
            WS_OVERLAPPEDWINDOW,
            _pos_x, _pos_y,
            _size_x, _size_y,
