...
public:
    ...
    const dxgi_component_t&                  get_dxgi() noexcept { return m_dxgi; }
...
private:
    ...
    sys::unique_com_t<ID3D11RenderTargetView1> m_render_target_view;
    ...
    bool                                       m_compile_shaders(sys::unique_com_t<ID3DBlob>& out_vs_blob, sys::unique_com_t<ID3DBlob>& out_ps_blob);
    bool                                       m_setup_input_layout(sys::unique_com_t<ID3DBlob>& in_vs_blob);
    //  vertex and pixel shaders
    sys::unique_com_t<ID3D11VertexShader>      m_vs = nullptr;
    sys::unique_com_t<ID3D11PixelShader>       m_ps = nullptr;

    bool                                       m_setup_vertex_buffer();
    ///  @brief describes how data is organized before being passed to vertex shader
    sys::unique_com_t<ID3D11InputLayout>       m_vertex_input_layout;
    ///  @brief an array of vertices described by 3 coordinates on the screen
    sys::unique_com_t<ID3D11Buffer>            m_current_vertex_buffer;
    ...
    sys::unique_com_t<ID3D11Buffer>            m_current_index_buffer;

    bool                                       m_setup_constant_buffer();
    ///  @brief holds matrices for rotating the cube
    sys::unique_com_t<ID3D11Buffer>            m_current_constant_buffer;
