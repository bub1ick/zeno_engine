...
public:
    ...
    utils::unique_com_t<ID3D11Device5>&        get_device() noexcept { return m_device; }

    utils::unique_com_t<ID3D11DeviceContext4>& get_device_context() noexcept { return m_device_context; }

    const dxgi_component_t&                    get_dxgi() noexcept { return m_dxgi; }

    void update(const sys::window_t& in_window, const float delta_time_in_seconds) noexcept;
...
private:
    ...
    void                                         m_update_rotation(const float delta_time_in_seconds) noexcept;
