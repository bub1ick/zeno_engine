...
private:
    ...
    utils::com_unique_ptr<ID3D11Device5>           m_device;
    ///  @brief the direct3d device context
    utils::com_unique_ptr<ID3D11DeviceContext4>    m_device_context;
    ...
    utils::com_unique_ptr<ID3D11RenderTargetView1> m_render_target_view;
    ...
    bool                                           m_compile_shaders(utils::com_unique_ptr<ID3DBlob>& out_vs_blob, utils::com_unique_ptr<ID3DBlob>& out_ps_blob);
    bool                                           m_setup_input_layout(utils::com_unique_ptr<ID3DBlob>& in_vs_blob);
    //  vertex and pixel shaders
    utils::com_unique_ptr<ID3D11VertexShader>      m_vs = nullptr;
    utils::com_unique_ptr<ID3D11PixelShader>       m_ps = nullptr;

    bool                                           m_setup_vertex_buffer();
    ///  @brief describes how data is organized before being passed to vertex shader
    utils::com_unique_ptr<ID3D11InputLayout>       m_vertex_input_layout;
    ///  @brief an array of vertices described by 3 coordinates on the screen
    utils::com_unique_ptr<ID3D11Buffer>            m_current_vertex_buffer;
    ...
    utils::com_unique_ptr<ID3D11Buffer>            m_current_index_buffer;

    bool                                           m_setup_constant_buffer();
    ///  @brief holds matrices for rotating the cube
    utils::com_unique_ptr<ID3D11Buffer>            m_current_constant_buffer;
